...
describe('AsyncResult', () => {
    ...
    describe('orElse', () => {
        it('should return mapped Ok for outer Err', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).orElse(() => Ok(2)), Ok(2));
        });

        it('should return mapped Ok for outer Err (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).orElse(async () => Ok(2)), Ok(2));
        });

        it('should return mapped Err for outer Err', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).orElse(() => Err('err')), Err('err'));
        });

        it('should return mapped Err for outer Err (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).orElse(async () => Err('err')), Err('err'));
        });
    ...
    describe('map', () => {
        it('should return mapped value for outer Ok', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Ok(1))).map(() => 2), Ok(2));
        });

        it('should return mapped value for outer Ok (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Ok(1))).map(async () => 2), Ok(2));
        });

        it('should return outer error for outer Err', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).map(() => 2), Err(1));
        });

        it('should return outer error for outer Err (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).map(async () => 2), Err(1));
        });
    });

    describe('mapErr', () => {
        it('should return mapped error for outer Err', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).mapErr(() => 2), Err(2));
        });

        it('should return mapped error for outer Err (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Err(1))).mapErr(async () => 2), Err(2));
        });

        it('should return outer ok for outer Ok', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Ok(1))).mapErr(() => 2), Ok(1));
        });

        it('should return outer ok for outer Ok (async)', async () => {
            assert.deepEqual(await new AsyncResultImpl(Promise.resolve(Ok(1))).mapErr(async () => 2), Ok(1));
        });
    });
